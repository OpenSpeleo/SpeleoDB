...
def assign_random_colors(apps, schema_editor):
    """Assign a random color to every existing GPS track."""
    GPSTrack = apps.get_model("gis", "GPSTrack")
    for track in GPSTrack.objects.all():
        track.color = random.choice(PALETTE)  # noqa: S311
        track.save(update_fields=["color"])


class Migration(migrations.Migration):
    dependencies = [
        ("gis", "0034_cylinder_manufactured_date_and_more"),
    ]
