        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if params.fix_dmp:
                ...
                try:
                    ...
                    # Before this hook existed the wrap middleware used to
                    # be the last-resort Sentry capture for exceptions that
                    # escaped the view. The middleware no longer runs for
                    # ``/api/v2/`` paths, so this view must surface its own
                    # failures explicitly. The ``correct_dmp_cmd`` helper
                    # wraps a third-party mnemo_lib call and can fail for
                    # both user-input reasons (corrupt DMP) and genuine
                    # bugs in our pipeline -- we log and capture in all
                    # cases because silent 400s are worse than a little
                    # Sentry noise.
                    logger.exception("ToolDMPDoctor correct_dmp_cmd failure")
