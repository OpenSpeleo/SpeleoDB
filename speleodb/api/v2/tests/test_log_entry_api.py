...
from speleodb.api.v2.tests.file_utils import create_test_image
from speleodb.api.v2.tests.file_utils import create_test_text_file
from speleodb.api.v2.tests.file_utils import create_test_video
...
class TestStationLogEntryCreateAPIValidation(BaseAPIProjectTestCase):
    ...
    def test_api_returns_validation_errors(self) -> None:
        ...
        response = self.client.post(
            reverse("api:v2:station-logs", kwargs={"id": self.station.id}),
            {
    ...
    def test_real_file_uploads(self, notes: str | None, attachment: bool) -> None:
        ...
        if attachment:
            assert "http" in response.data["attachment"]
        else:
            assert response.data["attachment"] is None
...
class TestStationLogEntryListAPI(BaseTestStationLogEntryGetAPI):
    ...
    def test_list_resources_with_data(self) -> None:
        ...
        logs_data = list(reversed(response.data))
