...
from speleodb.api.v2.tests.factories import CylinderPressureCheckFactory
...
class TestCylinderPressureCheckListCreate:
    ...
    def test_list_pressure_checks(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-install-pressure-checks",
        ...
        assert response.data[0]["id"] == str(pressure_check.id)
    ...
    def test_list_pressure_checks_multiple(
        ...
        pressures = [c["pressure"] for c in response.data]
    ...
    def test_create_pressure_check_valid(
        ...
        assert response.data["pressure"] == 2800  # noqa: PLR2004
        assert response.data["unit_system"] == UnitSystem.IMPERIAL
        assert response.data["check_date"] == "2025-01-15"
    ...
    def test_create_pressure_check_metric(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["pressure"] == 200  # noqa: PLR2004
        assert response.data["unit_system"] == UnitSystem.METRIC
...
class TestCylinderPressureCheckRetrieveUpdateDelete:
    ...
    def test_retrieve_pressure_check(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-pressure-check-detail",
                kwargs={
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(pressure_check.id)
        assert response.data["pressure"] == pressure_check.pressure
    ...
    def test_update_pressure_check(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["pressure"] == 2500  # noqa: PLR2004 - valid PSI
        assert response.data["notes"] == "Updated reading"
...
class TestCylinderPressureCheckValidation:
    ...
    def test_pressure_check_includes_install_info(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert "install_id" in response.data
        assert "cylinder_id" in response.data
        assert "cylinder_name" in response.data
    ...
    def test_full_update_put(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["pressure"] == 200  # noqa: PLR2004 - valid BAR
        assert response.data["notes"] == "Full update"
        assert response.data["check_date"] == "2025-01-20"
