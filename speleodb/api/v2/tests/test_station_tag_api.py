...
class TestStationTagAPI:
    ...
    def test_list_user_tags(
        ...
        url = reverse("api:v2:station-tags")
    ...
    def test_list_tags_only_shows_user_tags(
        ...
        assert response.data[0]["name"] == "Testtag"
    ...
    def test_create_station_tag(self, authenticated_client: APIClient) -> None:
        ...
        assert response.data["color"] == "#3B82F6"  # Uppercase
    ...
    def test_create_tag_with_lowercase_name(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["name"] == "High Priority"  # Capitalized
    ...
    def test_create_tag_with_extra_whitespace(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["name"] == "Active"  # Trimmed and capitalized
    ...
    def test_get_specific_tag(
        ...
        url = reverse("api:v2:station-tag-detail", kwargs={"id": tag.id})
        response = authenticated_client.get(url)

        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(tag.id)
        assert response.data["name"] == "Testtag"
    ...
    def test_update_station_tag(
        ...
        assert response.data["color"] == "#22C55E"
    ...
    def test_patch_station_tag(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == "Testtag"  # Unchanged
        assert response.data["color"] == "#22C55E"  # Changed
    ...
    def test_delete_station_tag(
        ...
        tag_id = tag.id
        url = reverse("api:v2:station-tag-detail", kwargs={"id": tag_id})
    ...
    def test_cannot_access_other_user_tag(
        ...
        url = reverse("api:v2:station-tag-detail", kwargs={"id": other_tag.id})
        response = authenticated_client.get(url)
    ...
    def test_get_predefined_colors(self, authenticated_client: APIClient) -> None:
        ...
        url = reverse("api:v2:station-tag-colors")
        response = authenticated_client.get(url)

        assert response.status_code == status.HTTP_200_OK
        assert "colors" in response.data
        assert len(response.data["colors"]) == 20  # noqa: PLR2004
        assert all(color.startswith("#") for color in response.data["colors"])
...
class TestStationTagManagementAPI:
    ...
    def test_get_station_tag(
        ...
        url = reverse("api:v2:station-tags-manage", kwargs={"id": station.id})
        response = authenticated_client.get(url)
    ...
    def test_change_station_tag(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == "Tag2"
