...
class TestCylinderFleetListCreate:
    ...
    def test_list_cylinder_fleets_authenticated(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-fleets"),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )

        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == n_fleet
    ...
    def test_create_cylinder_fleet_valid(
        ...
        assert response.data["name"] == "Test Fleet"
        assert response.data["created_by"] == user.email

        fleet_id = response.data["id"]
...
class TestCylinderFleetRetrieveUpdateDelete:
    ...
    def test_retrieve_cylinder_fleet_as_admin(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(cylinder_fleet_with_admin.id)
        assert response.data["name"] == cylinder_fleet_with_admin.name
    ...
    def test_retrieve_cylinder_fleet_no_permission(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-fleet-detail", kwargs={"fleet_id": fleet.id}),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
    ...
    def test_retrieve_nonexistent_fleet(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-fleet-detail", kwargs={"fleet_id": fake_uuid}),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
...
class TestCylinderListCreate:
    ...
    def test_list_cylinders_in_fleet(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-fleet-cylinders",
                kwargs={"fleet_id": cylinder_fleet_with_admin.id},
            ),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == n_cylinders
    ...
    def test_create_cylinder_valid(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["name"] == "New Cylinder"
    ...
    def test_create_cylinder_with_dates(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["manufactured_date"] == "2020-06-15"
        assert response.data["last_visual_inspection_date"] == "2023-03-10"
        assert response.data["last_hydrostatic_test_date"] == "2022-08-20"
    ...
    def test_create_cylinder_with_null_dates(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["manufactured_date"] is None
        assert response.data["last_visual_inspection_date"] is None
        assert response.data["last_hydrostatic_test_date"] is None
...
class TestCylinderRetrieveUpdateDelete:
    ...
    def test_retrieve_cylinder(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-detail", kwargs={"id": cylinder.id}),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )

        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(cylinder.id)
    ...
    def test_update_cylinder(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == "Updated Cylinder"
    ...
    def test_update_cylinder_dates(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["manufactured_date"] == "2019-01-15"
        assert response.data["last_visual_inspection_date"] == "2024-01-10"
        assert response.data["last_hydrostatic_test_date"] == "2023-06-20"
...
class TestCylinderFleetEdgeCases:
    ...
    def test_special_characters_in_names(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["name"] == "Fleet™ 测试 🚀"
...
class TestCylinderFleetExport:
    ...
    def test_export_cylinders_excel(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-fleet-cylinders-export",
                kwargs={"fleet_id": cylinder_fleet_with_read.id},
            ),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
...
class TestCylinderFleetWatchlist:
    ...
    def test_watchlist_default_days(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-fleet-watchlist",
                kwargs={"fleet_id": cylinder_fleet_with_read.id},
            ),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
        ...
        assert response.status_code == status.HTTP_200_OK
        cylinder_ids = {c["id"] for c in response.data}
...
class TestCylinderFleetWatchlistExport:
    ...
    def test_export_watchlist_excel(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-fleet-watchlist-export",
                kwargs={"fleet_id": cylinder_fleet_with_read.id},
            ),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
