...
class TestCylinderInstallListCreate:
    ...
    def test_list_cylinder_installs_authenticated(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-installs"),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )

        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]["id"] == str(cylinder_install.id)
    ...
    def test_list_cylinder_installs_filter_by_cylinder(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-installs") + f"?cylinder_id={cylinder.id}",
            HTTP_AUTHORIZATION=get_auth_header(user),
        )

        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]["id"] == str(install1.id)
    ...
    def test_list_cylinder_installs_filter_by_status(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-installs") + "?status=installed",
            HTTP_AUTHORIZATION=get_auth_header(user),
        )

        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]["status"] == InstallStatus.INSTALLED
    ...
    def test_create_cylinder_install_valid(
        ...
        assert response.data["cylinder_id"] == str(cylinder.id)
        assert response.data["project_id"] == str(project.id)
        assert response.data["location_name"] == "Test Cave"
        assert response.data["install_user"] == user.email
        assert response.data["status"] == InstallStatus.INSTALLED
    ...
    def test_create_cylinder_install_with_distance(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["distance_from_entry"] == 1500  # noqa: PLR2004
...
class TestCylinderInstallRetrieveUpdateDelete:
    ...
    def test_retrieve_cylinder_install(
        ...
        response = api_client.get(
            reverse(
                "api:v2:cylinder-install-detail",
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(cylinder_install.id)
        assert response.data["cylinder_id"] == str(cylinder_install.cylinder.id)
    ...
    def test_update_status_to_retrieved(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["status"] == InstallStatus.RETRIEVED
        assert response.data["uninstall_date"] == uninstall_date.isoformat()
        assert response.data["uninstall_user"] == user.email
    ...
    def test_update_status_to_retrieved_auto_fills_dates(
        ...
        assert response.data["status"] == InstallStatus.RETRIEVED
        assert response.data["uninstall_user"] == user.email
        assert response.data["uninstall_date"] is not None
    ...
    def test_update_status_to_lost(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["status"] == InstallStatus.LOST
    ...
    def test_update_status_to_abandoned(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["status"] == InstallStatus.ABANDONED
    ...
    def test_update_location_name(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["location_name"] == "Updated Location"
...
class TestCylinderInstallGeoJSON:
    ...
    def test_geojson_returns_feature_collection(
        ...
        response = api_client.get(
            reverse("api:v2:cylinder-installs-geojson"),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
...
class TestCylinderInstallEdgeCases:
    ...
    def test_pressure_check_count_in_response(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert "pressure_check_count" in response.data
        assert response.data["pressure_check_count"] == 0
