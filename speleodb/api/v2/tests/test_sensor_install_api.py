...
class TestStationSensorInstallListCreate:
    ...
    def test_list_sensor_installs_authenticated(
        ...
        response = api_client.get(
            reverse(
                "api:v2:station-sensor-installs",
        ...
        assert response.data[0]["id"] == str(sensor_install.id)
    ...
    def test_create_sensor_install_valid(
        ...
        assert response.data["sensor_id"] == str(sensor.id)
        assert response.data["station_id"] == str(station.id)
    ...
    def test_create_sensor_install_with_expiracy_dates(
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["expiracy_memory_date"] is not None
        assert response.data["expiracy_battery_date"] is not None
...
class TestStationSensorInstallRetrieveUpdate:
    ...
    def test_retrieve_sensor_install(
        ...
        response = api_client.get(
            reverse(
                "api:v2:station-sensor-install-detail",
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(sensor_install.id)
        assert response.data["sensor_id"] == str(sensor_install.sensor.id)
        assert response.data["station_id"] == str(station.id)
...
class TestStationSensorInstallHistory:
    ...
    def test_list_all_sensor_installs_without_status_filter(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 4  # noqa: PLR2004

        # Verify all statuses are present
        statuses = {install["status"] for install in response.data}
        assert statuses == {
...
class TestStationSensorInstallExcelExport:
    ...
    def test_export_excel_successful(
        ...
        response = api_client.get(
            reverse(
                "api:v2:station-sensor-installs-export",
                kwargs={"id": station.id},
            ),
            HTTP_AUTHORIZATION=get_auth_header(user),
        )
