...
class TestProjectInteraction(BaseAPIProjectTestCase):
    ...
    def test_get_user_project(self) -> None:
        ...
        response = self.client.get(
            reverse("api:v2:project-detail", kwargs={"id": self.project.id}),
        ...
        serializer = ProjectSerializer(data=response.data)
    ...
    def test_acquire_and_release_user_project(self) -> None:
        ...
        for _ in range(5):
            ...
            assert project_data == response.data, {
                "reserialized": project_data,
                "response_data": response.data,
            }
            assert response.data["active_mutex"]["user"] == self.user.email, (
                response.data,
                self.user.email,
            )
        ...
        for _ in range(5):
            auth = self.header_prefix + self.token.key
            response = self.client.post(
                reverse("api:v2:project-release", kwargs={"id": self.project.id}),
                headers={"authorization": auth},
            )
            ...
            assert response.data["active_mutex"] is None, response.data
    ...
    def test_acquire_and_release_user_project_with_comment(self) -> None:
        ...
        assert response.data["active_mutex"] is None, response.data["active_mutex"]
...
class TestProjectCreation(BaseAPITestCase):
    ...
    def test_create_project(self, use_lat_long: bool) -> None:
        ...
        assert is_subset(data, response.data), response.data
...
class TestProjectListLatestCommit(BaseAPITestCase):
    ...
    def test_project_list_includes_latest_commit_field(self) -> None:
        """Test that GET /api/v2/projects/ includes latest_commit field."""
    ...
    def test_project_list_latest_commit_structure(self) -> None:
        ...
        assert response.status_code == status.HTTP_200_OK
        project_data = next(p for p in response.data if p["id"] == str(project.id))
