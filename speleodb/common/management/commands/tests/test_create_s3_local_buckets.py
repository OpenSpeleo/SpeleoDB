...
class TestCreateS3LocalBucketsCommand(TestCase):
    def setUp(self) -> None:
        ...
        self.client_patcher = patch(
            "speleodb.common.management.commands.create_s3_local_buckets."
            "boto3.client"
        )
    ...
    def test_cors_error_names_the_affected_bucket(self) -> None:
        ...
        with pytest.raises(
            CommandError,
            match=(
                "Failed to apply CORS rules to bucket "
                "'speleodb-user-artifacts-dev'"
            ),
        ):
            call_command("create_s3_local_buckets")
...
class TestRustFSCORSIntegration(TestCase):
    def test_presigned_get_and_head_responses_allow_browser_origin(self) -> None:
        ...
        try:
            ...
            assert parse_qs(urlparse(get_url).query)["X-Amz-Algorithm"] == [
                "AWS4-HMAC-SHA256"
            ]
            get_request = Request(  # noqa: S310
                get_url,
                headers={"Origin": "http://localhost:8000"},
                method="GET",
            )
            with urlopen(get_request, timeout=5) as response:  # noqa: S310
            ...
            head_request = Request(  # noqa: S310
                head_url,
                headers={"Origin": "http://localhost:8000"},
                method="HEAD",
            )
            with urlopen(head_request, timeout=5) as response:  # noqa: S310
                assert response.headers["Access-Control-Allow-Origin"] == "*"
