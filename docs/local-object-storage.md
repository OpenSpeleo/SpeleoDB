...
Provisioning adds a few S3 control-plane calls at local startup only. It adds
no Django request-path work, no map feature rescans, and no production runtime
cost. RustFS evaluates the stored CORS rule while serving the object response.
