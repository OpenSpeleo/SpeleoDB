...
Use conversion only for source formats such as KML/KMZ that Mapbox cannot
consume directly.

Do not add endpoint throttling as an unsolicited review hardening measure.
Resource bounds and rate policy are different product decisions; a failed upload
must not unexpectedly lock a user out of the next attempt.
