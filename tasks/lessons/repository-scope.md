...
for a web-only task: those commands enter unrelated application repositories.
Use the web-local commands from `apps/web` and run that repository's own
`prek run -a`. Inspect another repository only when the user explicitly expands
the scope.
