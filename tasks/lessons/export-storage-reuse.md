...
locally. Put reusable transfer/version-cleanup operations on the shared storage
backend, and retain only export-specific policy in the export adapter.

Check both signing modes, persisted object keys, exact version deletion, and
expiry enforcement. Bucket policy must allow the configured CloudFront origin
while viewer access remains restricted by signed URLs.
